...
def get_files():
    try:
        ...
        if not os.path.exists(img_database):
            print(f"Image database not found: {img_database}")
            return jsonify({"error": "Image database not found"}), 404
        if not os.path.exists(audio_database):
            print(f"Audio database not found: {audio_database}")
            return jsonify({"error": "Audio database not found"}), 404
        ...
        for img in img_files:
            try:
                file_path = os.path.join(img_database, img)
                encrypted_path = os.path.join(encrypted_img_database, f"encrypt_{img}")
                
                # 如果加密文件不存在，先创建加密文件
                if not os.path.exists(encrypted_path):
                    try:
                        encrypt_image(file_path, encrypted_path)
                        print(f"Created encrypted image: {encrypted_path}")
                    except Exception as e:
                        print(f"Failed to encrypt image {img}: {str(e)}")
                        continue
        ...
        for audio in audio_files:
            try:
                file_path = os.path.join(audio_database, audio)
                encrypted_path = os.path.join(encrypted_audio_database, f"encrypt_{audio}")
                
                # 如果加密文件不存在，先创建加密文件
                if not os.path.exists(encrypted_path):
                    try:
                        encrypt_audio(file_path, encrypted_path)
                        print(f"Created encrypted audio: {encrypted_path}")
                    except Exception as e:
                        print(f"Failed to encrypt audio {audio}: {str(e)}")
                        continue
        ...
        if sort_by == 'size':
            ...
            files.sort(key=lambda x: x['date'], reverse=reverse)
        else:  # sort by name
        ...
        print(f"Returning {len(files)} files, total pages: {total_pages}")
        return jsonify({
            'files': files,
            'total': total,
